...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-[#111111] border border-[#2A2A2A] p-6 shadow-[0_4px_12px_rgba(0,0,0,0.4)]",
  {
    variants: {
      side: {
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Title
    ref={ref}
    className={cn("text-lg font-semibold text-[#CFCFF6] tracking-[-0.01em]", className)}
    {...props}
  />
))
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Description
    ref={ref}
    className={cn("text-sm text-[#8A8C8E] tracking-[-0.01em]", className)}
    {...props}
  />
))
