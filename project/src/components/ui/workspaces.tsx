...
// Gradient color sets for workspace avatars
...
// Generate consistent gradient based on workspace ID
const getWorkspaceGradient = (workspaceId: string): string => {
	...
	const index = Math.abs(hash) % WORKSPACE_GRADIENTS.length;
				...
				<div className="flex min-w-0 flex-1 items-center gap-2">
					<Avatar className="h-6 w-6">
						...
						<AvatarFallback 
							...
							style={{ 
								background: getWorkspaceGradient(getWorkspaceId(selectedWorkspace))
							}}
						>
		...
		<div className="flex min-w-0 flex-1 items-center gap-2">
			<Avatar className="h-6 w-6">
				...
				<AvatarFallback 
					className="text-xs text-white font-semibold"
					style={{ 
						background: getWorkspaceGradient(getWorkspaceId(workspace))
					}}
				>
