  ...
  return (
    ...
    >
      <motion.div
        ...
        className={cn(
          "fixed top-0 bottom-0 right-0 z-50 h-full w-[420px] bg-[#000000] backdrop-blur-md text-[#CFCFF6] border-l border-[#2A2A2A]/60 shadow-xl rounded-lg",
          className
        )}
        style={{
          left: leftPosition,
    ...
    try {
      ...
      try {
        ...
        if (success) {
          ...
          // Call completion callback to refresh project view if provided
          if (onUploadComplete) {
            console.log('📢 Calling onUploadComplete callback for project refresh');
            onUploadComplete();
          }
        }
      } catch (error) {
    ...
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      ...
      <AnimatePresence>
        ...
        <div className="flex flex-col h-full">
          {/* Header */}
          <SheetHeader className="p-6 pb-4 border-b border-[#000000] space-y-1">
            <SheetTitle className="text-white text-xl font-bold flex items-center justify-between">
              ...
              {contextUploads.filter(u => u.status === 'uploading').length > 0 && (
                <span className="text-xs font-normal text-[#4338CA] bg-[#4338CA]/10 px-2 py-1 rounded-md border border-[#4338CA]/30 flex items-center space-x-1">
          ...
          <div className="flex-1 p-6 space-y-4 overflow-y-auto">
            {/* Auto-tagging Toggle */}
            <div className="bg-[#000000]/50 border border-[#000000] rounded-lg p-4">
              <div className="flex items-center justify-between">
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  ...
                  />
                  <div className="w-11 h-6 bg-[#000000] peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-[#4338CA]/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#4338CA]"></div>
                </label>
              </div>
            </div>
            ...
            <div
              ...
              className={`border-2 border-dashed rounded-lg p-8 text-center transition-all duration-200 ${
                dragActive 
                  ? 'border-[#4338CA] bg-[#4338CA]/10' 
                  : 'border-[#000000] hover:border-[#4338CA]/50'
              }`}
            >
              <Upload className={`w-12 h-12 mx-auto mb-4 ${
                dragActive ? 'text-[#4338CA]' : 'text-[#CFCFF6]/40'
              }`} />
              ...
              <Button
                ...
                disabled={isUploading}
                variant="outline"
                className="border-2 border-[#4338CA] bg-[#4338CA]/20 text-[#CFCFF6] hover:bg-[#4338CA]/30 hover:text-white hover:border-[#4338CA] transition-all"
              >
                Choose Files
              </Button>
              <input
              ...
              <div>
                ...
                <div className="space-y-2 max-h-40 overflow-y-auto">
                  {selectedFiles.map((file, index) => (
                    <div key={`${file.name}-${index}`} className="flex items-center justify-between p-3 bg-[#000000]/50 rounded-lg">
                      ...
                      <Button
                        ...
                        variant="ghost"
                        size="icon"
                        className="h-8 w-8 flex-shrink-0 hover:bg-[#000000]"
                      >
                        <X className="w-4 h-4" />
                      </Button>
                    </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  {contextUploads.map((upload) => (
                    <div key={upload.id} className="p-3 bg-[#000000]/50 border border-[#2A2A2A]/30 rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-2 flex-1 min-w-0">
                          {upload.status === 'completed' ? (
                            ...
                            <Loader className="w-4 h-4 text-[#4338CA] animate-spin flex-shrink-0" />
                          )}
                      ...
                      <div className="w-full bg-[#000000] rounded-full h-2 mb-2 overflow-hidden">
                        <div
                          className={`h-2 rounded-full transition-all duration-500 ease-out ${
                            upload.status === 'failed' ? 'bg-red-500' : 
                            upload.status === 'completed' ? 'bg-green-500' : 
                            upload.status === 'paused' ? 'bg-[#CFCFF6]/40' : 
                            'bg-gradient-to-r from-[#4338CA] to-[#8b5cf6]'
                          }`}
                          style={{ 
                  ...
                  {contextUploads.length === 0 && uploads.map((upload) => (
                    <div key={upload.fileId} className="p-3 bg-[#000000]/50 border border-[#2A2A2A]/30 rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-2 flex-1 min-w-0">
            ...
            {recentUploads.length > 0 && (
              <div>
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto border border-[#000000] rounded-lg p-3 bg-[#000000]/20">
                  {recentUploads.map((file) => (
                    <div key={file.id} className="p-3 bg-[#000000]/50 rounded-lg">
                      <div className="flex items-center justify-between">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {file.tags && file.tags.length > 0 ? (
                              file.tags.map((tag) => (
                                <span
                                  key={tag}
                                  className="inline-flex items-center px-2 py-0.5 bg-[#000000] text-[#CFCFF6]/80 text-xs rounded-md"
                                >
                                  {tag}
                                </span>
                              ))
                        ...
                        <Button
                          ...
                          variant="ghost"
                          size="icon"
                          className="h-8 w-8 flex-shrink-0 text-[#CFCFF6]/60 hover:text-red-400 hover:bg-[#000000]"
                        >
                          <X className="w-4 h-4" />
                        </Button>
                      </div>
          ...
          {/* Footer */}
          <SheetFooter className="p-6 pt-4 border-t border-[#000000] flex-row justify-between items-center space-x-0">
            {errorMessage ? (
              <div className="text-red-400 text-sm flex-1">{errorMessage}</div>
            ) : contextUploads.filter(u => u.status === 'uploading').length > 0 ? (
              <div className="text-[#4338CA] text-sm flex-1 flex items-center space-x-2">
                <Loader className="w-3 h-3 animate-spin" />
                <span>
            ...
            <div className="flex items-center space-x-2 ml-auto">
              <Button
                onClick={() => onOpenChange(false)}
                variant="ghost"
                className="text-[#CFCFF6] hover:bg-[#000000]"
              >
              ...
              {selectedFiles.length > 0 && (
                <Button
                  ...
                  variant="outline"
                  className="border-2 border-[#4338CA] bg-[#4338CA]/20 text-[#CFCFF6] hover:bg-[#4338CA]/30 hover:text-white hover:border-[#4338CA] transition-all flex items-center space-x-2"
                >
